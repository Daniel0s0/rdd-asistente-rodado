...
import { webhookCasoModificacionHandler } from '@api/webhook';
...
describe('webhookCasoModificacionHandler', () => {
  ...
  it('rejects missing webhook signature', async () => {
    const body = { causa_id: 'test-123', rit: 'RIT-123' };
    const req = createRequest(body) as any;
    const res = createResponse() as any;

    await webhookCasoModificacionHandler(req, res);
  ...
  it('returns 404 if causa not found in DB', async () => {
    ...
    const body = { causa_id: 'nonexistent-causa', rit: 'RIT-123' };
    const signature = generateSignature(body);
    const req = createRequest(body, signature) as any;
    const res = createResponse() as any;
  ...
  it('updates conversation metadata on valid payload', async () => {
    ...
    const body = {
      causa_id: 'test-123',
      rit: 'RIT-2024-001',
      tribunal: 'Juzgado Santiago',
      cambios: { estado: 'apelacion' },
    };
    const signature = generateSignature(body);
    const req = createRequest(body, signature) as any;
    const res = createResponse() as any;
